 ...
 */
public class StreamTextIOTranslator {
   ...
   */
  @SuppressWarnings({"rawtypes", "unchecked"})
  public static class WriteTranslator implements TransformTranslator<StreamTextIO.Write.Bound> {
    @Override
    public void translate(
        StreamTextIO.Write.Bound transform,
        TranslationContext context) {
      translateWriteHelper(transform, context);
    }

    private <T> void translateWriteHelper(
        StreamTextIO.Write.Bound<T> transform,
        TranslationContext context) {
//      if (context.getPipelineOptions().isStreaming()) {
//        throw new IllegalArgumentException("TextIO not supported in streaming mode.");
//      }
